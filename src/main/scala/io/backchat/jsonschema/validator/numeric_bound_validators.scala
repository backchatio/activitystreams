  ...
  def validateValue(value: JValue, schema: JValue): Validation[ValidationError, JValue] = {
    ...
    (value, schema \ property) match {
      ...
      case (_: JInt | _: JFloat, _) =>
        ValidationError("The value %s is %s %s" % (generate(value), failMsg(isExclusive), generate(schema \ property)), property).fail
      case _ => ValidationError("The `%s` property can only contain numbers." % property, property).fail
    }
  }
